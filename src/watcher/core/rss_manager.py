...
class RSSManager:
    ...
    def _write_rss_feed(self, title: str, link: str, description: str, items: List[Dict]) -> None:
        ...
        for item in items:
            ...
            desc_elem.text = f"__CDATA_START__{item.get('description', '')}__CDATA_END__"
            
            # Add pubDate
            pubdate = item.get('pubdate')
            if isinstance(pubdate, datetime):
                ET.SubElement(item_elem, 'pubDate').text = pubdate.strftime('%a, %d %b %Y %H:%M:%S +0000')
