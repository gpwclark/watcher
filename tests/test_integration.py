...
class IntegrationTest:
    ...
    def verify_results(self, results):
        ...
        assert len(cycle1['files']) >= 3, f"Cycle 1: Expected at least 3 files, got {cycle1['files']}"
        
        # Cycle 2: One update (dynamic-article-1)
        cycle2 = results[1]
        print(f"Cycle 2: {len(cycle2['files'])} files")
        assert len(cycle2['files']) >= 4, f"Cycle 2: Expected at least 4 files (3 + 1 update), got {cycle2['files']}"
        assert cycle2['content_hash'] != cycle1['content_hash'], "Cycle 2: Content should have changed"
        
        # Cycle 3: Two updates (dynamic-article-1 and dynamic-article-2)
        cycle3 = results[2]
        print(f"Cycle 3: {len(cycle3['files'])} files")
        assert len(cycle3['files']) >= 6, f"Cycle 3: Expected at least 6 files (4 + 2 updates), got {cycle3['files']}"
        assert cycle3['content_hash'] != cycle2['content_hash'], "Cycle 3: Content should have changed"
        ...
        if static_dir.exists():
            ...
            assert len(static_files) == 1, f"Static article should have only 1 file, found {len(static_files)}"
